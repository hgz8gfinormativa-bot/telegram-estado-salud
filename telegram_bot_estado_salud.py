...
async def set_estado(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data["mode"] = "estado"
    await update.message.reply_text("Modo seleccionado: estado de salud. Ahora envía el PDF.")


async def set_resumen(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data["mode"] = "resumen"
    await update.message.reply_text("Modo seleccionado: resumen clínico. Ahora envía el PDF.")


async def set_cronologia(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data["mode"] = "cronologia"
    await update.message.reply_text("Modo seleccionado: cronología médica. Ahora envía el PDF.")


async def set_estado_familiar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data["mode"] = "estado_familiar"
    await update.message.reply_text("Modo seleccionado: estado para familiar. Ahora envía el PDF.")
...
async def set_estado_autoridad(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Modo seleccionado: estado para autoridad. Ahora envía el PDF.")
...
async def set_estado_institucional(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Modo seleccionado: estado institucional. Ahora envía el PDF.")
...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f"Procesando PDF en modo: {mode}")
    ...
    try:
        ...
        if not text.strip():
            ...
            if OCR_ENABLED and not OCR_LIBS_AVAILABLE:
                msg += " OCR está activado, pero faltan librerías del sistema o de Python."
            await update.message.reply_text(msg)
            return

        if len(text.strip()) < MIN_TEXT_LENGTH:
            await update.message.reply_text(
                "El PDF no contiene suficiente texto legible para generar un resultado confiable."
            )
            return
        ...
        if "429" in error_text or "quota" in error_text or "rate limit" in error_text:
            await update.message.reply_text(
                "No fue posible procesar el documento por límite de cuota o velocidad del servicio de IA. Intenta nuevamente más tarde."
            )
        else:
            await update.message.reply_text(f"Ocurrió un error al procesar el PDF: {e}")
